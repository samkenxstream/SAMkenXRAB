...
describe('DiscoveryDocUpdate', () => {
  it('should return false in checkPR if incoming PR does not match classRules', async () => {
    ...
    assert.deepStrictEqual(await discoveryDocUpdate.checkPR(pullRequest), false);
  });

  it('should return true in checkPR if incoming PR does match classRules', async () => {
    const pullRequest: PullRequest = {
      author: 'googleapis-publisher',
      title: 'chore: autogenerated discovery document update NOW',
      fileCount: 3,
      changedFiles: [{filename: 'README.md', sha: '2345'}],
      repoName: 'testRepoName',
      repoOwner: 'testRepoOwner',
      prNumber: 1,
      body: 'body',
    ...
    assert.ok(await discoveryDocUpdate.checkPR(pullRequest));
  });
});
